...
class ChromstemDataset(Dataset):
    ...
    def __init__(self, csv_file, root_dir, frac=1.0, transform=None):
        ...
        self.nucl_coords_frame_ = pd.read_csv(
            csv_file, engine="python", header=None, names=col_names, skiprows=skip
        )
    ...
    def read_chromstem(self, fnme):
        ...
        rho_factor = (
            27.00 / rho.max()
        )  # Normalize in the same way the experimentalists do
    ...
    def __getitem__(self, idx):
        ...
        chromstem_name = os.path.join(
            self.root_dir_, self.nucl_coords_frame_.iloc[idx, 0]
        )
        ...
        sample = {
            "chromstem": chromstem,
            "nucl_coords": nucl_coords,
            "num_nucls": num_nucls - 1,
        }
